...
int main( int argc, char* argv[]){
        ...
        while( !feof(f) ) {
                ...
                *p++ = '\0' ;
                if(!strstr(line, "MEMINFO:") ) continue;
...
inline int
parseLine(const char text[], nod_t * res, int * type) {
        char op[128]={0};       /* operator ( new or delete )*/
        char file[128] = {0 };  /* file */
        int  ptr=0 ;            /* pointer */
        char* p=0;              /* used to find the line Number */
        int  sz=0;              /* size allocated */
        int r =0;
        short int line=0;
