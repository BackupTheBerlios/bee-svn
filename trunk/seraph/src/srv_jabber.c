...
static LmSSLResponse ssl_cb( LmSSL * ssl, LmSSLStatus status, gpointer ud )
{
        ...
        case LM_SSL_STATUS_CERT_HOSTNAME_MISMATCH:
                g_printerr
                    ( "TestLM: Certificate hostname does not match expected hostname!\n" );
                break;
        case LM_SSL_STATUS_CERT_FINGERPRINT_MISMATCH:{
                        const char *fpr = lm_ssl_get_fingerprint( ssl );
                        g_printerr
                            ( "TestLM: Certificate fingerprint does not match expected fingerprint!\n" );
...
connection_close_cb( LmConnection * connection,
                     LmDisconnectReason reason, gpointer user_data )
{
        const char *str;
...
handle_messages( LmMessageHandler * handler,
                 LmConnection * connection, LmMessage * m, gpointer user_data )
{
        LmMessage *mout;
        GError *error = NULL;
        gchar *msg = NULL;
        ...
        msg = lm_message_node_find_child( m->node, "body" )->value;
        ...
        lm_message_node_add_child( mout->node, "body", rsp );

        if( !lm_connection_send( connection, mout, &error ) ) {
                g_printerr( "LmSendAsync: Failed to send message:'%s'\n",
                            lm_message_node_to_string( mout->node ) );
        } else {
                g_print( "LmSendAsync: Sent message:'%s'\n",
                         lm_message_node_to_string( mout->node ) );
        }

        lm_message_unref( mout );
...
int main( int argc, char **argv )
{
        ...
        if( fingerprint ) {
                ...
                for( i = 0, p = fingerprint; *p && *( p + 1 ); i++, p += 3 ) {
                        expected_fingerprint[i] =
                            ( unsigned char )g_ascii_strtoull( p, NULL, 16 );
                }
        ...
        if( !result ) {
                g_printerr
                    ( "TestLM: Opening connection failed, error:%d->'%s'\n",
