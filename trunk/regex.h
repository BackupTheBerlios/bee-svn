...
class RegEx
{
     ...
     public:
          ...
          bool Search( const char* subject, int len =-1, int options =0) {
               ClearMatchList();
               return pcre_exec( re, pe, lastsubject =subject,
                                 slen =( len >= 0)  ? len : strlen( subject),
                                 0, options, ovector, 3*substrcount)  > 0;
          } //  Search
          ...
          const char * Match( int i = 1 ) {
               ...
               if( matchlist == NULL) {
                    pcre_get_substring_list( lastsubject, ovector,
                              substrcount, &matchlist);
               }
               return matchlist[ i ];
          } //  Match
     ...
     private:
          inline void ClearMatchList( void) {
               if( matchlist)
                    pcre_free_substring_list( matchlist), matchlist = NULL;
          }
};
#endif
