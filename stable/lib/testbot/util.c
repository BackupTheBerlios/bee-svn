...
int util_matches( char* buffer, char* string ){
    if(!strncasecmp(buffer, string, strlen(string) ) )return 1;
    return 0 ;
}
...
int my_rm(char *srcName)
{
    ...
    } else
    {
        if (recursiveAction(srcName, recursiveFlag, FALSE,
                            TRUE, rm_fileAction, rm_dirAction, NULL) == FALSE)
        {
            exit(FALSE);
        }
    }
                             ...
                             const char* cfgFile, const char* crash_destDir )
{
    ...
    if(!dir) { fprintf(stderr, "Can't open %s : %s\n", core_srcDir, strerror(errno) ); exit(-1) ;}

    while ( ( core = readdir(dir) ) )
    {
        if ( strstr(core->d_name,"core" )) {
            sprintf( dst, "%s/data/crash-%s", crash_destDir, core->d_name ) ;
            rc = mkdir( dst, 0777 ) ;
            if(rc == -1 ) {
                fprintf(stderr,"Can't create core folder %s/data/crash-%s\n",
